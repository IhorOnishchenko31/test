...
const checkConnect = async (page, emailAddress, PASSWORD) => {
  try {
    ...
    await retry(() =>
      page.goto(
        "https://www.upwork.com/jobs/~021843763796333372491",{
...
const getProxy = async () => {
  try {
    const proxies = await fs.readFile("proxyList.txt", 'utf-8');
...
const startScript = async () => {
  ...
  while (true) {
    ...
    try {
      ...
      if(hasConnect){
        sendSlackMessage(`${emailAddress}    =>  ${PASSWORD} =>  ${proxies[proxyIndex]}`);
      }
      updateStatus(
